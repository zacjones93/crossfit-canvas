  ...
  return (
    <section className="relative h-screen min-h-[70vh] lg:min-h-[80vh] flex items-center justify-center overflow-hidden pt-3">
      ...
      <div className="absolute inset-0 z-0">
        {/* Hero Background Image - prefers-reduced-motion fallback */}
        ...
        {/* Hero Background Video - All devices (respects prefers-reduced-motion) */}
        <video
          autoPlay
          loop
          muted
          playsInline
          className="motion-reduce:hidden absolute inset-0 w-full h-full object-cover grayscale"
        >
      ...
      <div className="relative z-20 container mx-auto px-6 text-center max-w-5xl">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          ...
          <Link
            href="/schedule"
            className="group text-white text-base md:text-lg font-medium px-4 py-2 transition-all"
          >
            <span className="relative inline-block">
              View Class Schedule
              {/* Red brush stroke underline */}
              <span className="absolute left-0 right-0 -bottom-1 h-[3px] bg-white/80 transform origin-left transition-transform group-hover:scale-x-110" />
            </span>
            <span className="ml-1 text-white/80 transition-transform group-hover:translate-x-1 inline-block">
              →
            </span>
          </Link>
        </div>
